...
def train_masif_site(
    ...
    data_dirs = data_dirs
    n_val = len(training_list) // 10
    val_dirs = set(training_list[(len(training_list) - n_val) :])
    # print("Validation set: {}".format(val_dirs))
    ...
    for num_iter in range(num_iterations):
        ...
        all_test_labels = []
        all_test_scores = []
        all_test_labels_convert = []
        all_test_scores_convert = []

        for ppi_pair_id in data_dirs:
            cases=[]
            if ppi_pair_id+"_1" in training_list:
                cases.append("1")
            if ppi_pair_id+"_2" in training_list:
                cases.append("2")
            if len(cases) == 0:
                continue
            ...
            for case in cases:
                try:
                    iface_labels = np.load(mydir + pid + f"_iface_labels{case}.npy")
                    # iface_labels2 = np.load(mydir + pid + "_iface_labels2.npy")
                except:
                    print("No labels for {}".format(mydir + pid))
                    exit()
                ...
                pos_labels = np.where(iface_labels >= 0.9)[0]
                neg_labels = np.where(iface_labels < 0.9)[0]
                ...
                if ppi_pair_id+f"_{case}" in val_dirs:
                    logfile.write("Validating on {} {}\n".format(ppi_pair_id+f"_{case}", pid))
                    ...
                    # if np.isnan(training_loss).any():
                    #     print("Nan training loss. Skipping protein.")
                    #     continue
                    # print(norm_grad)
    ...
    # for ppi_pair_id in data_dirs:
    #     cases=[]
    #     if ppi_pair_id+"_1" in testing_list:
    #         cases.append("1")
    #     if ppi_pair_id+"_2" in testing_list:
    #         cases.append("2")
    #     if len(cases) == 0:
    #         continue
    #     mydir = params["masif_precomputation_dir"] + ppi_pair_id + "/"
    ...
    #         tmp = np.zeros((len(iface_labels), 2))
    #         for i in range(len(iface_labels)):
    #             if iface_labels[i] >=0.75:
    #                 tmp[i, 0] = 1
    #             else:
    #                 tmp[i, 1] = 1
    #         iface_labels_dc = tmp
    #         logfile.flush()
    #         pos_labels = np.where(iface_labels >=0.75)[0]
    #         neg_labels = np.where(iface_labels <0.25)[0]
    ...
    #         score = score[0]
    #         print(score)
    #         iface_labels_convert = np.zeros(len(iface_labels))
    #         score_convert = np.zeros(len(iface_labels))
    #         for i in range(len(iface_labels)):
    #             if iface_labels[i] >=0.75:
    #                 iface_labels_convert[i] = True
    #             else:
    #                 iface_labels_convert[i] = False
    #             if score[i] >=0.75:
    #                 score_convert[i] = True
    #             else:
    #                 score_convert[i] = False
            
    #         auc = metrics.roc_auc_score(iface_labels_convert, score_convert)
            
    #         list_test_auc.append(auc)
    #         list_test_names.append((ppi_pair_id+f"_{case}", pid))
    #         all_test_labels.append(iface_labels)
    #         all_test_labels_convert.append(iface_labels_convert)
    #         all_test_scores_convert.append(score_convert)

    # outstr += "Per protein AUC mean (test): {:.4f}; median: {:.4f} \n".format(
    #     np.mean(list_test_auc), np.median(list_test_auc)
    # )
    # flat_all_test_labels = np.concatenate(all_test_labels_convert, axis=0)
    # flat_all_test_scores = np.concatenate(all_test_scores_convert, axis=0)
    ...
    #     learning_obj.saver.save(learning_obj.session, output_model)
        ...
        # np.save(out_dir + "test_labels.npy", all_test_labels_convert)
        # np.save(out_dir + "test_scores.npy", all_test_scores_convert)

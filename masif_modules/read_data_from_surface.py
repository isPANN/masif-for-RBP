...
def read_data_from_surface(ply_fn, params):
    ...
    RNAbind1=mesh.get_attribute("vertex_RNA_binding1")
    RNAbind2=mesh.get_attribute("vertex_RNA_binding2")
    ...
    rbd_labels = np.zeros((n, params['max_shape_size'], 2))
    ...
    for vix in range(n):
        ...
        rbd_labels[vix, :len(neigh_vix), 0] = RNAbind1[neigh_vix]
        rbd_labels[vix, :len(neigh_vix), 1] = RNAbind2[neigh_vix]
        
    return input_feat, rho, theta, mask, neigh_indices, rbd_labels, np.copy(mesh.vertices)
...
#     D = np.squeeze(np.asarray(coord[np.int(vix), : coord.shape[1] // 2].todense(),dtype=object))
...
#     patch["RNAbind1"]=shape["RNAbind1"][0][j]
#     patch["RNAbind2"]=shape["RNAbind2"][0][j]
...
#     theta = np.squeeze(np.asarray(coord[np.int(vix), j_theta].todense(),dtype=object))

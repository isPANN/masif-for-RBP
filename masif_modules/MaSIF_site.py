    ...
    def __init__(
        ...
        with tf.Graph().as_default() as g:
            ...
            with tf.device(idx_gpu):
                ...
                # self.eval_score = tf.clip_by_value(tf.squeeze(self.eval_logits)[:, 0], tf.keras.backend.epsilon(), 1 - tf.keras.backend.epsilon())

                # self.full_logits = tf.nn.sigmoid(self.logits)
                # self.full_score = tf.clip_by_value(tf.squeeze(self.full_logits)[:, 0], tf.keras.backend.epsilon(), 1 - tf.keras.backend.epsilon())

                # self.data_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                #     labels=tf.to_float(self.eval_labels),
